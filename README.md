# AutoTrader — 한국투자증권 자동매매 프로그램

Python + KIS API 기반 한국·미국 주식 자동매매 프로그램입니다.  
웹 UI를 통해 전략 전환, 종목 탐색 모드 설정, 실시간 시세 확인, 매매 내역 관리, 전략별 백테스트를 제공합니다.

---

## 목차

1. [사전 준비](#1-사전-준비)
2. [설치](#2-설치)
3. [API 키 설정](#3-api-키-설정)
4. [실행 방법](#4-실행-방법)
5. [UI 화면 구성](#5-ui-화면-구성)
6. [거래 시장 설정](#6-거래-시장-설정)
7. [전략 설명](#7-전략-설명)
8. [종목 탐색 모드](#8-종목-탐색-모드)
9. [감시 종목](#9-감시-종목)
10. [매매 제외 종목](#10-매매-제외-종목)
11. [매수/매도 내역](#11-매수매도-내역)
12. [백테스트](#12-백테스트)
13. [리스크 관리](#13-리스크-관리)
14. [설정 파일 안내](#14-설정-파일-안내)
15. [파일 구조](#15-파일-구조)
16. [주의사항](#16-주의사항)

---

## 1. 사전 준비

| 항목 | 내용 |
|------|------|
| Python | 3.10 이상 |
| 한국투자증권 계좌 | 실전 또는 모의투자 계좌 |
| KIS Developers 가입 | https://apiportal.koreainvestment.com |
| API 키 | AppKey + SecretKey (KIS Developers에서 발급) |

---

## 2. 설치

```bash
pip install -r requirements.txt
```

---

## 3. API 키 설정

`.env.example`을 복사해 `.env` 파일을 만들고 실제 값을 입력합니다.

```bash
cp .env.example .env
```

```dotenv
KIS_ID=HTS_로그인_아이디
KIS_ACCOUNT=계좌번호-01          # 예: 50012345-01

KIS_APPKEY=발급받은_AppKey
KIS_SECRETKEY=발급받은_SecretKey

# 모의투자 사용 시 추가
KIS_VIRTUAL_APPKEY=모의투자_AppKey
KIS_VIRTUAL_SECRETKEY=모의투자_SecretKey
```

**API 키 발급 순서**

1. https://apiportal.koreainvestment.com 접속 후 로그인
2. **My Page → 앱 등록** 클릭
3. 앱 이름 입력 → 실전투자 선택 → 등록
4. 생성된 **AppKey**, **AppSecret** 복사 후 `.env`에 입력

> AppKey 유효기간은 **90일**입니다. 만료 전 KIS Developers에서 갱신하세요.

---

## 4. 실행 방법

### 방법 A — 웹 UI 포함 실행 (권장)

```bash
python app.py
```

브라우저에서 **http://localhost:5000** 접속 후 **시작하기** 버튼 클릭.

### 방법 B — 터미널 단독 실행

```bash
python main.py
```

`yes` 입력 후 시작. 로그는 터미널과 `trading.log` 파일에 동시 기록됩니다.

---

## 5. UI 화면 구성

```
┌─────────────────────────────────────────────────────────────┐
│  AutoTrader   [● 실행 중]            실전투자  [중지하기]     │  헤더
├─────────────────────────────────────────────────────────────┤
│  계좌 현황                                             [↺]  │
│  [현금 잔고 KRW]  [USD 잔고]  [주식 평가금액]  [총 자산]     │  잔고 카드 (4개)
├─────────────────────────────────────────────────────────────┤
│  보유종목  (3)                                              │
│  종목코드 | 수량 | 평균단가 | 현재가 | 수익률 | 평가손익     │  보유종목 테이블
├─────────────────────────────────────────────────────────────┤
│  감시 종목  (25)                                       [↺]  │
│  ── KR 한국 주식 (15) ──────────────────────────────────  │
│  종목 | 현재가(원) | 등락률 | 거래량 | 스크리너             │
│  ── US 미국 주식 (10) ──────────────────────────────────  │
│  종목 | 현재가($)  | 등락률 | 거래량 | 스크리너             │
├─────────────────────────────────────────────────────────────┤
│  매매 제외 종목  (1)                                        │
│  [종목코드 입력]  [추가]                                    │  제외 종목 관리
│  ORCL ×                                                    │
├─────────────────────────────────────────────────────────────┤
│  매수/매도 내역  (12)                                       │
│  [오늘]  [1주일]  [1개월]  [전체]                          │  기간 필터 탭
│  일시 | 종목 | 구분 | 사유 | 상세                          │  매매 내역 테이블
├─────────────────────────────────────────────────────────────┤
│  전략 설정  (dark)                                         │
│  [RSI]  [이동평균 크로스]  [볼린저밴드]  [변동성 돌파]      │  전략 선택
│  실행 주기: 5분  |  투자 모드: 실전  |  KR [감시 목록]     │
│                                          US [전체 시장]     │  종목 탐색 토글
├─────────────────────────────────────────────────────────────┤
│  백테스트                                                  │
│  [종목코드] [KR ▼] [6개월 ▼] [실행]                       │  조회 조건
│  전략 | 총수익률 | 거래수 | 승률 | 평균수익 | 최대낙폭     │  결과 테이블
├─────────────────────────────────────────────────────────────┤
│  실행 로그                                      [초기화]   │
│  10:30:01  INFO  [005930] 매수 1주 @ 72,500원             │  실시간 로그
└─────────────────────────────────────────────────────────────┘
```

### 봇 시작 / 중지

- 헤더 우측 **시작하기** 클릭 → 자동매매 시작
- 실행 중에는 버튼이 **빨간색 "중지하기"** 로 변경
- 봇이 실행 중일 때는 전략 변경 및 탐색 모드 변경이 차단됩니다

### 자동 갱신 주기

| 항목 | 주기 |
|------|------|
| 봇 상태 | 3초 |
| 실행 로그 | 5초 |
| 매수/매도 내역 | 10초 |
| 잔고 / 보유종목 | 60초 |
| 감시 종목 시세 | 60초 |

---

## 6. 거래 시장 설정

`config.yaml`의 `market` 항목으로 거래 대상 시장을 선택합니다.

```yaml
market: BOTH   # KR: 한국만 / US: 미국만 / BOTH: 한국+미국
```

| 값 | 동작 |
|----|------|
| `KR` | 한국 주식만 거래 (KRX) |
| `US` | 미국 주식만 거래 (NASDAQ / NYSE / AMEX) |
| `BOTH` | 한국 + 미국 동시 거래 |

- 한국 장 운영 시간: **평일 09:00 ~ 15:30 KST**
- 미국 장 운영 시간: **평일 09:30 ~ 16:00 ET** (서머타임 자동 적용)
- 각 시장이 닫혀 있으면 해당 시장 매매는 자동 건너뜁니다

### 미국 종목 설정 형식

```yaml
us_screener:
  min_volume: 1000000
  max_stocks: 10
  watchlist:
    - "AAPL.NASD"    # Apple — NASDAQ
    - "MSFT.NASD"    # Microsoft
    - "NVDA.NASD"    # NVIDIA
    - "TSLA.NASD"    # Tesla
    - "META.NYSE"    # Meta — NYSE (예시)
```

거래소 코드: `NASD` (NASDAQ) · `NYSE` (뉴욕거래소) · `AMEX` (아메리칸거래소)

---

## 7. 전략 설명

전략은 UI에서 클릭 한 번으로 전환할 수 있습니다 (봇 중지 상태에서).  
한국·미국 모두 동일한 전략이 적용됩니다 (미국은 일봉 기준).

### RSI (상대강도지수)

- RSI가 **30 이하** 과매도 구간에서 반등 → 매수
- RSI가 **70 이상** 과매수 구간에서 하락 → 매도
- 횡보장에서 효과적

```yaml
rsi:
  period: 14      # RSI 계산 기간
  oversold: 30    # 매수 기준값
  overbought: 70  # 매도 기준값
```

### 이동평균 크로스오버 (MA Cross)

- 단기 이평선이 장기 이평선을 **위로 돌파** → 매수 (골든크로스)
- 단기 이평선이 장기 이평선을 **아래로 돌파** → 매도 (데드크로스)
- 추세 추종형 전략

```yaml
ma_cross:
  short_period: 5    # 단기 이동평균 기간
  long_period: 20    # 장기 이동평균 기간
```

### 볼린저밴드 (Bollinger Bands)

- 주가가 하단 밴드 이탈 후 **회복** → 매수
- 주가가 상단 밴드 이탈 후 **하락** → 매도
- 변동성이 큰 종목에 효과적

```yaml
bollinger:
  period: 20      # 기간
  std_dev: 2.0    # 표준편차 배수 (밴드 폭)
```

### 변동성 돌파 (Volatility Breakout)

- 매수가 = 당일 시가 + (전일 고가 − 전일 저가) × k
- 현재가가 매수가를 돌파하면 → 매수
- 당일 매수 후 장 마감 전 전량 매도
- 래리 윌리엄스 전략 기반, 단순하고 실전 검증된 방식

```yaml
volatility_breakout:
  k: 0.5   # 변동성 계수 (0.3 ~ 0.7 권장)
```

---

## 8. 종목 탐색 모드

봇이 매수 후보를 어디서 찾을지 결정합니다. UI **전략 설정** 섹션의 **KR / US 토글**로 시장별로 독립 설정합니다.

### 감시 목록 (기본)

등록된 `watchlist` 종목만 검사합니다.

- 빠르고 예측 가능
- 원하는 종목을 직접 관리

### 전체 시장 자동 탐색

시장 전체에서 전략 조건에 맞는 종목을 자동 선정합니다.

| 시장 | 동작 |
|------|------|
| **KR** | KIS 거래량 순위 API 조회 → 스크리너 필터 적용 |
| **US** | KIS 해외주식 조건검색 API 조회 → 실패 시 NASDAQ·NYSE 주요 60종목 내장 목록으로 폴백 |

```yaml
use_dynamic_universe:    true   # KR 전체 시장 탐색
use_dynamic_universe_us: true   # US 전체 시장 탐색

screener:
  min_universe_volume: 100000   # KR 유니버스 최소 거래량

us_screener:
  min_universe_volume: 500000   # US 유니버스 최소 거래량
```

- 감시 목록보다 사이클당 처리 시간이 더 걸릴 수 있습니다
- KR과 US를 각각 독립적으로 활성화/비활성화할 수 있습니다

---

## 9. 감시 종목

UI의 **감시 종목** 섹션에서 `watchlist`에 등록된 종목들의 현재 시세를 실시간으로 확인할 수 있습니다.  
한국 주식과 미국 주식이 별도 섹션으로 분리되어 표시됩니다.

| 컬럼 | 설명 |
|------|------|
| 종목코드 / 종목명 | 종목 식별 정보 |
| 현재가 | 실시간 주가 (KR: 원화, US: 달러) |
| 등락률 | 전일 대비 등락률 (상승 초록 / 하락 빨강) |
| 거래량 | 당일 누적 거래량 |
| 스크리너 | **통과** (초록) / **거래량 부족** / **가격 범위 초과** (회색) |

- 시세 조회 불가 시에도 종목 코드와 이름은 항상 표시됩니다
- `↺` 버튼으로 즉시 새로고침, 또는 60초마다 자동 갱신

```yaml
screener:
  watchlist:
    - "005930"   # 삼성전자
    - "000660"   # SK하이닉스

us_screener:
  watchlist:
    - "AAPL.NASD"   # Apple
    - "MSFT.NASD"   # Microsoft
```

---

## 10. 매매 제외 종목

특정 종목을 자동매매에서 완전히 제외할 수 있습니다.  
UI의 **매매 제외 종목** 섹션에서 실시간으로 추가·삭제할 수 있으며, `excluded.json` 파일에 영구 저장됩니다.

- 제외 목록은 KR / US 스크리너 양쪽 모두에 적용됩니다
- 전체 시장 탐색 모드에서도 제외 종목은 매수 대상에서 제외됩니다
- 봇 재시작 후에도 설정이 유지됩니다

**UI 사용법**

1. 종목코드 입력란에 코드를 입력 (예: `ORCL`, `005930`)
2. **추가** 버튼 클릭 또는 Enter 키
3. 추가된 종목은 빨간 칩으로 표시 — **×** 버튼으로 제거

**직접 파일 편집 (`excluded.json`)**

```json
["ORCL", "005930"]
```

---

## 11. 매수/매도 내역

자동매매로 실행된 모든 매수·매도 기록을 확인할 수 있습니다.  
`trades.json` 파일에 저장되어 **앱 재시작 후에도 내역이 유지**됩니다.

### 기간 필터

| 탭 | 표시 범위 |
|----|----------|
| 오늘 | 당일 00:00 이후 |
| 1주일 | 최근 7일 |
| 1개월 | 최근 30일 |
| 전체 | 전체 내역 |

### 내역 컬럼

| 컬럼 | 설명 |
|------|------|
| 일시 | 매매 실행 날짜·시간 (`YYYY-MM-DD HH:MM`) |
| 종목 | 종목코드 |
| 구분 | **매수** (초록) / **매도** (빨강) |
| 사유 | 매수 · 매도 · 손절 · 익절 |
| 상세 | 수량·가격 또는 수익률 |

---

## 12. 백테스트

전략별 과거 성과를 시뮬레이션해 비교할 수 있습니다.  
UI의 **백테스트** 섹션에서 종목·시장·기간을 선택한 뒤 **실행**을 클릭하면 4개 전략의 결과가 한 번에 표시됩니다.

### 동작 방식

- KIS API로 일봉 데이터를 조회한 뒤, 각 봉마다 해당 시점까지의 데이터를 전략에 전달해 신호를 생성합니다
- 신호 발생 봉의 **종가**에 체결하며, 초기 자본 1,000만 원 전액을 1회 매수에 사용합니다
- 기간 종료 시 미청산 포지션은 자동 정리됩니다
- 실제 매매와 동일한 전략 파라미터(`config.yaml`)를 사용합니다

### 조회 조건

| 항목 | 선택지 |
|------|--------|
| 종목코드 | 직접 입력 (예: `005930`, `AAPL`) |
| 시장 | KR (KRX) · US NASDAQ · US NYSE |
| 기간 | 3개월 · 6개월 · 1년 |

### 결과 지표

| 지표 | 설명 |
|------|------|
| 총 수익률 | 백테스트 기간 전체 수익률 (초기 자본 대비) |
| 거래수 | 총 매도 완료 횟수 |
| 승률 | 수익 거래 비율 |
| 평균 수익 | 거래당 평균 수익률 |
| 최대 낙폭 | 고점 대비 최대 손실폭 (MDD) |

- 결과 상단에 **매수보유(Buy & Hold) 수익률**을 함께 표시해 전략 성과를 비교할 수 있습니다
- 가장 높은 수익률을 기록한 전략에 **최고** 배지가 표시됩니다

> 백테스트는 과거 데이터 기반 시뮬레이션으로, 미래 수익을 보장하지 않습니다.

---

## 13. 리스크 관리

매 사이클마다 보유 종목에 대해 손절 / 익절을 자동 체크합니다.

```yaml
risk:
  max_buy_amount: 500000        # KR 종목당 1회 최대 매수금액 (원)
  us_max_buy_amount_usd: 300    # US 종목당 1회 최대 매수금액 (USD)
  stop_loss_pct: -5.0           # -5% 손실 시 자동 손절
  take_profit_pct: 10.0         # +10% 수익 시 자동 익절
```

- `max_buy_amount` / `us_max_buy_amount_usd`를 낮게 설정할수록 분산 투자 효과
- 손절 / 익절은 한국·미국 보유 종목 모두에 적용됩니다

---

## 14. 설정 파일 안내

### config.yaml

```yaml
# 전략 선택
strategy: rsi   # rsi | ma_cross | bollinger | volatility_breakout

# 거래 시장
market: BOTH   # KR | US | BOTH

# 종목 탐색 모드 (KR / US 독립 설정)
use_dynamic_universe:    false   # KR: false: 감시 목록 / true: 전체 시장
use_dynamic_universe_us: false   # US: false: 감시 목록 / true: 전체 시장

# 투자 모드
virtual: false   # true: 모의투자 / false: 실전투자

# 매매 주기 (분)
interval_minutes: 5

# 한국 스크리너 설정
screener:
  min_universe_volume: 100000  # KR 동적 탐색 시 유니버스 최소 거래량
  min_volume: 500000           # 스크리너 최소 거래량
  min_price: 5000              # 최소 주가 (원)
  max_price: 200000            # 최대 주가 (원)
  max_stocks: 5                # 최대 선택 종목 수
  watchlist:
    - "005930"                 # 삼성전자
    - "000660"                 # SK하이닉스

# 미국 스크리너 설정
us_screener:
  min_universe_volume: 500000  # US 동적 탐색 시 유니버스 최소 거래량
  min_volume: 1000000          # 스크리너 최소 거래량
  max_stocks: 10               # 최대 선택 종목 수
  watchlist:
    - "AAPL.NASD"              # Apple
    - "MSFT.NASD"              # Microsoft

# 리스크 관리
risk:
  max_buy_amount: 500000
  us_max_buy_amount_usd: 300
  stop_loss_pct: -5.0
  take_profit_pct: 10.0

# 전략 파라미터
rsi:
  period: 14
  oversold: 30
  overbought: 70

ma_cross:
  short_period: 5
  long_period: 20

bollinger:
  period: 20
  std_dev: 2.0

volatility_breakout:
  k: 0.5
```

---

## 15. 파일 구조

```
auto-trading/
├── .env                        ← API 키 (절대 공유 금지)
├── .env.example                ← API 키 템플릿
├── config.yaml                 ← 전략 · 시장 · 탐색 모드 · 파라미터 설정
├── excluded.json               ← 매매 제외 종목 목록 (UI에서 관리)
├── trades.json                 ← 매수/매도 내역 (자동 생성, 영구 저장)
├── trading.log                 ← 실행 로그 (자동 생성)
├── requirements.txt
├── README.md
│
├── app.py                      ← Flask 웹 서버 (UI 포함 실행 진입점)
├── main.py                     ← 터미널 단독 실행 진입점
├── kis_client.py               ← KIS API 래퍼 (시세·주문·KR/US 유니버스 조회)
├── screener.py                 ← 조건 검색 (감시 목록 / KR·US 동적 유니버스)
├── backtest.py                 ← 전략별 백테스트 엔진 (봉 단위 시뮬레이션)
│
├── strategies/
│   ├── __init__.py             ← 전략 로더
│   ├── base.py                 ← 전략 추상 기반 클래스
│   ├── rsi.py                  ← RSI 전략
│   ├── ma_cross.py             ← 이동평균 크로스
│   ├── bollinger.py            ← 볼린저밴드
│   └── volatility_breakout.py ← 변동성 돌파
│
├── templates/
│   └── index.html              ← 웹 UI 메인 페이지
│
└── static/
    ├── style.css               ← UI 스타일
    └── script.js               ← 프론트엔드 로직
```

---

## 16. 주의사항

> **실전투자는 실제 금전적 손실이 발생할 수 있습니다.**

- 처음 사용 시 반드시 **모의투자(`virtual: true`)** 로 테스트 후 실전투자로 전환하세요
- `max_buy_amount` / `us_max_buy_amount_usd`를 낮게 설정해 리스크를 제한하세요
- API 키는 절대 타인과 공유하거나 Git에 커밋하지 마세요 (`.gitignore`에 `.env` 포함)
- AppKey는 **90일마다 갱신**이 필요합니다
- KIS API 호출 제한: **1초당 20건, 1일 10만 건**
- 한국 장 운영 시간(평일 09:00 ~ 15:30 KST) 외에는 한국 주식 주문이 실행되지 않습니다
- 미국 장 운영 시간(평일 09:30 ~ 16:00 ET) 외에는 미국 주식 주문이 실행되지 않습니다
- 전체 시장 탐색 모드는 많은 종목을 조회하므로 사이클당 처리 시간이 길어질 수 있습니다
- 백테스트는 과거 데이터 기반 시뮬레이션이며, 실제 수익을 보장하지 않습니다
- 프로그램 종료 시 봇도 함께 중지됩니다. 장시간 운용 시 서버 또는 항상 켜둔 PC에서 실행하세요
